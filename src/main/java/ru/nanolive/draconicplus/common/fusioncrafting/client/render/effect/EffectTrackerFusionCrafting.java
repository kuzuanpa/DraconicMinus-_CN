...
public class EffectTrackerFusionCrafting {
    ...
    public boolean positionLocked = false, addedCoreParticle = false;
    ...
    public EffectTrackerFusionCrafting(World world, Vec3D pos, Vec3D corePos, IFusionCraftingInventory craftingInventory) {
        ...
        fx = new ParticleFusionCrafting(this.worldObj, this.pos, this.corePos, craftingInventory);
    }
    ...
    public void onUpdate(boolean isMoving) {
        ...
        if (!addedCoreParticle) {
            addedCoreParticle = true;
            DPEffectHandler.effectRenderer.addEffect(ResourceHelperDP.getResource("textures/blocks/fusion_crafting/fusion_particle.png"), fx);
        } else if(worldObj instanceof WorldClient){
            fx.setPosition(this.pos.x, this.pos.y - 0.5, this.pos.z);
        }
    ...
    public void renderEffect(Tessellator tessellator, float partialTicks) {
        ...
        GL11.glPopMatrix();
        this.prevPos.set(new BlockPos(relativeX,relativeY,relativeZ));
    }
